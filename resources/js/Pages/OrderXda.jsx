...
export default function OrderSultan({ user }) {
    // State Management
    ...
    // Live Polling Data Produk (Otomatis ambil data terbaru tiap 8 detik)
    useEffect(() => {
        fetchProduk();
        const interval = setInterval(() => fetchProduk(), 8000);
    ...
    // FOKUS KUNCI: Logika Matematika Anti-Prank (Pastikan jadi Angka/Integer)
            ...
            try {
                ...
                if (response.data.status === 'success') {
                    Swal.fire('Berhasil! 🎉', response.data.message, 'success').then(() => {
                        window.location.reload(); // Refresh untuk update saldo & riwayat
                    });
                } else {
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 sm:p-6 bg-gray-50 min-h-screen pb-32">
            {/* Header Saldo */}
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-xl mb-6 transform transition-all hover:scale-[1.01]">
                <h2 className="text-xl font-bold opacity-90">Saldo Anda</h2>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-md mb-6 border border-gray-100">
                <label className="block text-gray-700 font-bold mb-2 text-lg">Nomor Tujuan / Target</label>
                <input 
                    type="number" 
            ...
            {/* Grid Produk */}
            <h3 className="text-2xl font-bold text-gray-800 mb-4">Pilihan Layanan Sultan 💎</h3>
                    ...
                    {produkList.map((item) => {
                        ...
                        return (
                            <div 
                                ...
                                `}
                            >
                                <div className="font-bold text-gray-800 text-lg mb-1">{item.nama_layanan}</div>
                                <div className="text-blue-600 font-extrabold text-xl mb-3">
                                    Rp {hargaInt.toLocaleString('id-ID')}
                                </div>
                                
                                <div className="flex justify-between items-center text-sm font-semibold mt-auto">
                                    {isKosong ? (
                                        <span className="bg-red-100 text-red-600 py-1 px-3 rounded-full">● Habis / Gangguan</span>
                                    ) : (
                                        <span className="bg-green-100 text-green-600 py-1 px-3 rounded-full">⚡ Stok Aman</span>
                                    )}
                                </div>
                            </div>
                        );
            ...
            {/* Tombol Checkout Melayang (Sticky Bottom) */}
            <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-50">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div>
                        <p className="text-gray-500 text-sm font-semibold">Total Bayar</p>
                        <p className="text-2xl font-black text-gray-800">
                            {selectedProduk ? `Rp ${parseInt(selectedProduk.harga_jual).toLocaleString('id-ID')}` : 'Rp 0'}
                        </p>
                    </div>
                    <button 
