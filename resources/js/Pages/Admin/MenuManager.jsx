...
export default function MenuManager({ auth, adminMenus, appMenus, webSettings }) {
    ...
    const handleOrderChange = (id, value) => setLocalOrders({ ...localOrders, [id]: parseInt(value) || 0 });
    ...
    const handleEditMenu = async (menu) => {
        ...
        if (result.isConfirmed && result.value) {
            ...
            try {
                // Bungkus data menggunakan FormData agar File Gambar ter-upload sempurna
    ...
    const handleCreateMenu = async () => {
        const { value: formValues, isConfirmed } = await Swal.fire({
            ...
            preConfirm: () => {
                ...
                if (!name) { Swal.showValidationMessage('Nama menu wajib diisi!'); return false; }
                return {
                    id: null,
        ...
        if (isConfirmed && formValues) {
            ...
            try {
                ...
                Swal.fire({ icon: 'success', title: 'Menu Ditambahkan!', showConfirmButton: false, timer: 1000 });
                router.reload();
            } catch (e) { Swal.fire('Error', 'Gagal mendaftar', 'error'); }
        }
    };
    ...
    const renderIcon = (iconString) => {
        if (!iconString) return <i className="fa-solid fa-circle text-lg"></i>;
        if (iconString.includes('/') || iconString.includes('http')) {
            return <img src={iconString} alt="icon" className="w-8 h-8 object-contain rounded shadow-sm" />;
        }
        return <i className={`fa-solid ${iconString} text-xl`}></i>;
    };

    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="CMS Navigasi Menu" />
            <div className="p-6 bg-slate-900 min-h-screen text-white font-sans">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-800 pb-6">
                    <div>
                        <h2 className="font-black text-2xl tracking-tighter uppercase text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400">Pusat Kendali Navigasi v12</h2>
                        <p className="text-xs text-slate-400 font-bold mt-1">Klik langsung pada menu di bawah ini untuk mengedit nama, logo, jalur, atau menghapusnya secara instan.</p>
                    </div>
                    <button onClick={handleCreateMenu} className="bg-gradient-to-r from-emerald-500 to-green-600 text-white px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:scale-105 transition-transform">
                        ➕ Buat Menu Baru
                    </button>
                </div>

                <div className="flex flex-wrap gap-2 mb-8 bg-slate-800/60 p-2 rounded-2xl border border-slate-700/50 w-max">
                    <button onClick={() => setActiveTab('all')} className={`px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'all' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>✨ Semua Menu</button>
                    <button onClick={() => setActiveTab('app')} className={`px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'app' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>📱 Menu User ({appMenus.length})</button>
                    <button onClick={() => setActiveTab('admin')} className={`px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'admin' ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:text-white'}`}>🛡️ Menu Admin ({adminMenus.length})</button>
                    <button onClick={() => setActiveTab('identity')} className={`px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'identity' ? 'bg-amber-500 text-slate-900 shadow-md' : 'text-slate-400 hover:text-white'}`}>⚙️ Identitas Web</button>
                </div>

                {activeTab === 'identity' ? (
                    <div className="bg-slate-800 rounded-3xl border border-slate-700 p-6 max-w-xl">
                        <h3 className="font-black text-sm uppercase tracking-wider text-slate-400 mb-6">⚙️ Pengaturan Identitas Web</h3>
                        <form onSubmit={saveWebSettings}>
                            <div className="mb-5">
                                ...
                                <input type="text" value={webName} onChange={(e) => setWebName(e.target.value)} className="w-full border-2 border-slate-700 bg-slate-900 rounded-xl p-3 font-bold text-white focus:border-amber-500 focus:ring-0" required />
                            </div>
                            <div className="mb-6">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Logo Website</label>
                                {webSettings?.logo && (
                                    <div className="mb-3 p-3 bg-slate-900 rounded-xl w-max border border-slate-700">
                                        <img src={webSettings.logo} alt="Logo" className="h-12 object-contain" />
                                    </div>
                                )}
                                <input type="file" accept="image/*" onChange={(e) => setWebLogo(e.target.files[0])} className="w-full border-2 border-slate-700 bg-slate-900 rounded-xl p-2 font-bold text-slate-400 file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-black file:bg-amber-500 file:text-slate-900 hover:file:bg-amber-400 cursor-pointer" />
                            </div>
                            <button type="submit" className="bg-amber-500 text-slate-900 px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg hover:bg-amber-400 w-full transition-colors">Simpan Pengaturan</button>
                        </form>
                    </div>
                ) : (
                        ...
                        {(activeTab === 'all' || activeTab === 'app') && (
                            <div>
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <h3 className="font-black text-sm uppercase tracking-widest text-slate-400">Deretan Menu Aplikasi (User)</h3>
                                </div>
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 bg-slate-800/30 p-5 rounded-3xl border border-slate-800">
                                    {appMenus.map((menu) => (
                                        <div key={menu.id} onClick={() => handleEditMenu(menu)} className="flex flex-col items-center group cursor-pointer bg-slate-800/50 p-4 rounded-2xl border border-slate-700/40 hover:border-indigo-500/80 hover:bg-slate-800 transition-all transform hover:-translate-y-1">
                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-md relative overflow-hidden mb-2 ${menu.bg || 'bg-indigo-600'}`}>
                                                {renderIcon(menu.icon)}
                                            </div>
                                            <span className="text-[10px] font-black text-slate-300 text-center leading-tight tracking-tight group-hover:text-white line-clamp-1">{menu.name}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {(activeTab === 'all' || activeTab === 'admin') && (
                            <div>
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <h3 className="font-black text-sm uppercase tracking-widest text-slate-400">Deretan Menu Kendali (Admin)</h3>
                                </div>
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 bg-slate-800/30 p-5 rounded-3xl border border-slate-800">
                                    {adminMenus.map((menu) => (
                                        <div key={menu.id} onClick={() => handleEditMenu(menu)} className="flex flex-col items-center group cursor-pointer bg-slate-800/50 p-4 rounded-2xl border border-slate-700/40 hover:border-rose-500/80 hover:bg-slate-800 transition-all transform hover:-translate-y-1">
                                            <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-slate-700 text-slate-200 border border-slate-600 group-hover:bg-gradient-to-br group-hover:from-rose-500 group-hover:to-red-600 group-hover:text-white group-hover:border-transparent transition-all mb-2">
                                                {renderIcon(menu.icon)}
                                            </div>
                                            <span className="text-[10px] font-black text-slate-300 text-center leading-tight tracking-tight group-hover:text-white line-clamp-1">{menu.name}</span>
                                        </div>
