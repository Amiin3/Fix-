...
function ManageMenus({ auth }) {
  ...
  const renderIconPreview = (iconString) => {
    if (!iconString) return /* @__PURE__ */ jsx("i", { className: "fa-solid fa-circle" });
    if (iconString.includes("/") || iconString.includes("http")) return /* @__PURE__ */ jsx("img", { src: iconString, className: "w-full h-full object-contain p-1.5 drop-shadow-md", alt: "icon" });
    return /* @__PURE__ */ jsx("i", { className: `fa-solid ${iconString}` });
  };
  const handleEdit = (type, index) => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        ...
        if (type === "admin") {
          setAdminMenus(list);
          saveToServer(list, appMenus);
        } else {
          setAppMenus(list);
          saveToServer(adminMenus, list);
        }
      }
    });
  };
  const renderList = (type, data) => /* @__PURE__ */ jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6", children: data.map((m, i) => /* @__PURE__ */ jsxs("div", { className: "bg-white p-4 rounded-[20px] shadow-sm border border-slate-100 flex items-center justify-between group hover:border-blue-300 transition-all hover:shadow-md", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-4", children: [
      /* @__PURE__ */ jsx("div", { className: `w-14 h-14 rounded-2xl flex items-center justify-center text-2xl text-white shadow-inner overflow-hidden ${type === "app" ? m.bg : "bg-slate-800"}`, children: renderIconPreview(m.icon) }),
      /* @__PURE__ */ jsxs("div", { children: [
        /* @__PURE__ */ jsx("h4", { className: "font-black text-slate-800 text-sm leading-tight mb-0.5", children: m.name }),
        /* @__PURE__ */ jsx("p", { className: "text-[9px] text-slate-400 font-bold uppercase truncate max-w-[120px] bg-slate-50 px-2 py-0.5 rounded border border-slate-100", children: m.route || m.url })
      ] })
    ] }),
    /* @__PURE__ */ jsx("button", { onClick: () => handleEdit(type, i), className: "w-10 h-10 rounded-full bg-slate-50 border border-slate-200 text-blue-600 hover:bg-blue-600 hover:text-white transition-all shadow-sm flex items-center justify-center shrink-0", children: /* @__PURE__ */ jsx("i", { className: "fa-solid fa-pen" }) })
  ] }, i)) });
  return /* @__PURE__ */ jsxs(AuthenticatedLayout, { user: auth.user, children: [
    /* @__PURE__ */ jsx(Head, { title: "Kelola Menu" }),
    /* @__PURE__ */ jsx("div", { className: "py-8 bg-slate-50 min-h-screen font-['Outfit']", children: /* @__PURE__ */ jsxs("div", { className: "max-w-7xl mx-auto sm:px-6 lg:px-8", children: [
      /* @__PURE__ */ jsxs("div", { className: "bg-white p-6 rounded-[24px] shadow-sm border border-slate-200 mb-6 flex flex-col md:flex-row justify-between items-center gap-4", children: [
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-4", children: [
          /* @__PURE__ */ jsx("div", { className: "w-12 h-12 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center text-xl", children: /* @__PURE__ */ jsx("i", { className: "fa-solid fa-layer-group" }) }),
          /* @__PURE__ */ jsxs("div", { children: [
            /* @__PURE__ */ jsx("h2", { className: "text-xl font-black text-slate-800 leading-tight", children: "Manajer Menu Dinamis" }),
            /* @__PURE__ */ jsx("p", { className: "text-xs text-slate-500 font-medium", children: "Ubah logo dan warna menu secara instan." })
          ] })
        ] }),
        /* @__PURE__ */ jsxs("div", { className: "flex bg-slate-100 p-1.5 rounded-2xl w-full md:w-auto", children: [
          /* @__PURE__ */ jsx("button", { onClick: () => setActiveTab("app"), className: `flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "app" ? "bg-blue-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-200"}`, children: "Menu Produk" }),
          /* @__PURE__ */ jsx("button", { onClick: () => setActiveTab("admin"), className: `flex-1 md:flex-none px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === "admin" ? "bg-slate-900 text-white shadow-md" : "text-slate-500 hover:bg-slate-200"}`, children: "Menu Admin" })
        ] })
      ] }),
